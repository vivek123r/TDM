...
import { formatBytes, formatSpeed, formatDuration, generateProgressBar } from '../utils/format';
...
async function main() {
  ...
  let activeItems: DownloadItem[] = [];
  let queueItems: DownloadItem[] = [];
  let currentStats: DownloadStats | undefined;

  let activePanel: 'active' | 'queue' = 'active';
  let searchQuery = '';
  let filterMode: 'all' | 'completed' = 'all';
  ...
  // New Download Dialog
  const newDownloadForm = blessed.box({
    parent: screen,
    label: ' New Download ',
    border: 'line',
    width: 60,
  ...
  function getSelectedItem(): DownloadItem | null {
    if (activePanel === 'active') {
      const idx = (activeList as any).selected;
      if (idx >= 0 && idx < activeItems.length) {
        return activeItems[idx];
      }
    } else {
      const idx = (queueList as any).selected;
  ...
  function formatActiveRow(item: DownloadItem, width: number): string {
    ...
    if (item.status === 'completed') {
      ...
      statusText = item.eta ? `ETA ${formatDuration(item.eta)}` : 'ETA --:--';
      symbolColor = '{green-fg}';
    }
    ...
    const progressText = generateProgressBar(item.progress, 12);
  ...
  function formatQueueRow(item: DownloadItem, width: number): string {
  ...
  function renderLists() {
    ...
    if (searchQuery) {
      ...
      filtered = downloads.filter(
        (item) =>
          item.filename.toLowerCase().includes(q) || item.url.toLowerCase().includes(q)
      );
    }
    ...
    // Active List Render
    const activeWidth = activeList.width as number;
    const formattedActive = activeItems.map((item) => formatActiveRow(item, activeWidth));
    const prevActiveSelected = (activeList as any).selected;
    ...
    // Queue List Render
    const queueWidth = queueList.width as number;
    const formattedQueue = queueItems.map((item) => formatQueueRow(item, queueWidth));
    const prevQueueSelected = (queueList as any).selected;
  ...
  function updateHeader() {
    ...
    const contentWidth = screen.width as number;
  ...
  function updateStatusBar() {
    ...
    if (apiClient.connectionStatus && currentStats) {
      ...
      ram = formatBytes(currentStats.ramUsageBytes, 0).replace(' ', '');
      net = `↓${formatSpeed(currentStats.networkDownloadSpeed)}`;
  ...
  function openNewDownloadPopup() {
    urlInput.setValue('');
    saveDirInput.setValue(config.defaultDownloadDir);
  ...
  function openDetailsPopup() {
    ...
    const content = [
      ...
      `{bold}Progress:{/} ${item.progress}% (${formatBytes(item.downloadedBytes)} / ${
        item.totalBytes ? formatBytes(item.totalBytes) : 'Unknown'
      })`,
      `{bold}Speed:{/}    ${formatSpeed(item.speed)}`,
      `{bold}ETA:{/}      ${item.eta ? formatDuration(item.eta) : '--:--'}`,
